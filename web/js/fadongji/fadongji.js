const FenZi=function(){
	...
	this.init=function () {
		...
		function add3dCom(){
			...
			var p=new THREE.Vector3();p.x=0;p.y=0.3;p.z=0;	//new THREE.Vector3();

			common.loadFbxModel('models/fbx/fadongji6.fbx',p,0.06,com3d,false);//缩放后桌子尺寸1.56*1.07*0.91
		...
		this.transformRootChange=function(json){
	  	...
	  	this.transformChildChange=function(data){
	  		var tempobj=com3d.getObjectByName(data.targetName);//getObjectById(data.targetID);
	  		if(tempobj.position)
		...
		var rootParam={animal:true,scale:0.5,px:-2.5,py:0.5};
		//上一步
		...
		function addEventListeners(){
	        ...
	        $("#nextStep").click(myself.nextStep);
		  	...
		  	$("#big").click(function(){
		  		rootParam.scale=rootParam.scale+1;
		  		myself.setRootScale(rootParam.scale);
				if(mode=="teacher"){
					var json={
						event:"changescale",			
